...
export async function generateStaticParams() {
  const dates = await getAllTabDates("performance");
  return dates.map(date => ({ date }));
}

export default async function PerformanceDatePage({ params }) {
  const { date } = params;
  const [digest, allDates] = await Promise.all([
    getTabDigest("performance", date),
  ...
  if (idx < allDates.length - 1) {
    const prev = await getTabDigest("performance", allDates[idx + 1]);
