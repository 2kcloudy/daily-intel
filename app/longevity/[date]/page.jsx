...
export async function generateStaticParams() {
  const dates = await getAllTabDates("longevity");
  return dates.map(date => ({ date }));
}

export default async function LongevityDatePage({ params }) {
  const { date } = params;
  const [digest, allDates] = await Promise.all([
    getTabDigest("longevity", date),
  ...
  if (idx < allDates.length - 1) {
    const prev = await getTabDigest("longevity", allDates[idx + 1]);
