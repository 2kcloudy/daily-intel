...
export async function generateStaticParams() {
  const dates = await getAllTabDates("tech");
  return dates.map(date => ({ date }));
}

export default async function TechDatePage({ params }) {
  const { date } = params;
  const [digest, allDates] = await Promise.all([
    getTabDigest("tech", date),
    getAllTabDates("tech"),
  ]);
  ...
  if (idx < allDates.length - 1) {
    const prev = await getTabDigest("tech", allDates[idx + 1]);
