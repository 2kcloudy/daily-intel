...
export const metadata = {
  title: TAB_CONFIGS["energy"].metaTitle,
  description: TAB_CONFIGS["energy"].metaDescription,
};

export default async function EnergyPage() {
  const [digest, allDates] = await Promise.all([
    getLatestTabDigest("energy"),
  ...
  if (allDates.length > 1) {
    const prev = await getTabDigest("energy", allDates[1]);
