...
export async function generateStaticParams() {
  const dates = await getAllTabDates("crypto");
  return dates.map(date => ({ date }));
}

export default async function CryptoDatePage({ params }) {
  const { date } = params;
  const [digest, allDates] = await Promise.all([
    getTabDigest("crypto", date),
    getAllTabDates("crypto"),
  ]);
  ...
  if (idx < allDates.length - 1) {
    const prev = await getTabDigest("crypto", allDates[idx + 1]);
    if (prev?.stories) prevTopics = [...new Set(prev.stories.map(s => s.topic).filter(Boolean))];
  }

  return <GenericDigestView digest={digest} allDates={allDates} prevTopics={prevTopics} config={TAB_CONFIGS["crypto"]} />;
}
