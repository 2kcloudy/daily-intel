 ...
 * Mutates each story to include .image (and .imageSeed) if not already set.
 * Runs synchronously — no network, no KV.
 * Call this inside POST handlers BEFORE saving the digest.
 */
export function attachImagesToStories(stories = []) {
  for (const story of stories) {
    ...
    const seed     = storyImageSeed(headline, rank);
    if (!story.image) story.image = buildStoryImageUrl(headline, tag, rank);
