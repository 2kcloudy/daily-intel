...
export function buildPrompt(headline, tag) {
  const theme = TAG_THEMES[(tag || "").toLowerCase().replace(/[\s/+]+/g, "")]
    || "business news";
  // Use up to 100 chars of headline so images reflect the actual story
  const shortHeadline = (headline || "").slice(0, 100).trim();
  return (
    `Photorealistic editorial photograph: "${shortHeadline}". ` +
    `Visual context: ${theme}. ` +
    `Style: award-winning photojournalism, sharp focus, natural cinematic lighting, ` +
    `real people or real-world objects, no text overlays, no graphic design elements, ` +
    `high resolution, vivid but professional colour palette`
  );
}
...
//
// `force=true` skips the cache check and runs the full
// Pollinations → Blob → KV pipeline even if the entry exists. Used by the
// admin backfill to upgrade Pollinations-cached entries to permanent Blob URLs.
export async function generateAndCacheImage(seed, headline, tag, w = 600, h = 400, force = false) {
  const kvKey = imgKvKey(seed, w, h);

  // Already cached? Honour cache unless caller asked for a forced rebuild.
  if (!force) {
    try {
  ...
  try {
    ...
    // Pollinations or Blob threw — try again next request
  }

  // Only persist a successful Blob URL. If Pollinations is rate-limited (429)
  // or we couldn't upload, leave KV empty so the next caller retries the
  // upload instead of permanently caching a Pollinations URL that may keep
  // failing for downstream readers.
  if (blobUrl) {
    try { await kv.set(kvKey, blobUrl); } catch {}
    return blobUrl;
  }
