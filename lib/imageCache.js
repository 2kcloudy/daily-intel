...
export async function generateAndCacheImage(seed, headline, tag, w = 600, h = 400) {
  ...
  // Build Pollinations URL — use fast turbo model for backfill reliability
  // flux is highest quality but ~25s/image; turbo is ~3s/image
  const prompt = buildPrompt(headline, tag);
  const pollinationsUrl =
    `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}` +
    `?width=${w}&height=${h}&seed=${seed}&nologo=true&model=turbo`;

  // Fetch the image (60s timeout for reliability)
  let imgBuffer;
  try {
